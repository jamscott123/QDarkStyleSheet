...
def main(arguments):
    ...
    for ui_file in glob.glob('*.ui'):
        ...
        if args.create in ['pyqt', 'pyqtgraph', 'all']:
            try:
                call(['pyuic4', '--import-from=jam_darkstyle', ui_file, '-o', py_file_pyqt])
        ...
        if args.create in ['pyqt5', 'qtpy', 'all']:
            try:
                call(['pyuic5', '--import-from=jam_darkstyle', ui_file, '-o', py_file_pyqt5])
            except Exception as er:
        ...
        if args.create in ['pyside', 'all']:
            try:
                call(['pyside-uic', '--import-from=jam_darkstyle', ui_file, '-o', py_file_pyside])
            except Exception as er:
        ...
        if args.create in ['pyside2', 'all']:
            try:
                call(['pyside2-uic', '--import-from=jam_darkstyle', ui_file, '-o', py_file_pyside2])
            except Exception as er:
